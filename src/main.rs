...
async fn fetch_release(client: &Client, base_url: &str) -> Result<serde_json::Value, Box<dyn std::error::Error>> {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(expected) = expected_checksum {
        if !verify_checksum(&zip_bytes, &expected) {
            return Err(format!("Checksum mismatch: expected {}, got {}", expected, calculate_sha256(&zip_bytes)).into());
        }
