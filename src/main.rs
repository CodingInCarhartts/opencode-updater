...
use clap::Parser;
use opencode_updater::{Args, run_update, VersionManager, fetch_all_releases, fetch_release_by_tag, 
                      format_release_notes, load_cached_releases, cache_releases, display_version_comparison};
use ureq::Agent;
...
fn handle_rollback(version_manager: &VersionManager, version: &str) -> Result<(), Box<dyn std::error::Error>> {
    version_manager.rollback_to(version)
}

/// Handle list versions command
fn handle_list_versions(version_manager: &VersionManager, client: &ureq::Agent) -> Result<(), Box<dyn std::error::Error>> {
...
fn handle_compare(client: &ureq::Agent, versions: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    if versions.len() != 2 {
...
fn format_version_list(
    ...
    if let Some(curr) = current {
        output.push_str(&format!("✅ Current: {} (installed: {})\n\n", 
            curr.version, 
    ...
    if !installed.is_empty() {
        ...
        for version in installed {
            let marker = if current.as_ref().map_or(false, |c| c.version == version.version) {
                "→"
            } else {
                " "
            };
